...
// Fix gamma* and find how theta* varies with A
...
int main(){
	...
	// Create the file
	std::ofstream strm;
	strm.open("hmap_theta_vs_A.csv", std::ofstream::out|std::ofstream::trunc);

	for(A = 250; A < 750; A += 50) {
		...
		for(theta = 0.3; theta > -0.31; theta -= 0.05) {
			...
			int hbMidInitial = A; // Tanh centered at A
