...
// Fix gamma/nu* and find how theta* varies with nu
...
int main(){
	...
	// Create the file
	std::ofstream strm;
	strm.open("hmap_theta_vs_nu.csv", std::ofstream::out|std::ofstream::trunc);

	for(double nuexp = -5.0; nuexp <= -2.5; nuexp += 0.25) {
		nu = std::pow(10, nuexp); // nu = 10^nuexp
